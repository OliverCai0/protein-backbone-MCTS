...
import sys
import pymesh
#sys.path.append('/home/ilutz/support_scaffold/from_nick') # hopefully will work without this -- untested
...
def test_builder(num_runs = 80000001):
    ...
    while ct < num_runs:
        ...
        # if ct % 25000 == 0 and len(g.builds) > 0:
...
class tree_mesh_builder():
    def __init__(self, volume_mesh, stub, wnum_hash=None):
        ...
        self.min_ext_length = 0 # loops have partial helices on them, 4*2 loops generally
        ...
        self.second_ord_wf = 0 # add this to upweight downstream additions even more -- 0.1 good start
        self.starting_wf = 10
        ...
        #new data structures    
        self.builds = [] #vector of atom positions
        self.build_paths = [] #vector of extension lengths and loop bins

        #new sampling arrays, starting at zero, need to add min_length to extension try
        self.extension_sample = [i for i in range(0,self.max_ext_length-self.min_ext_length+1)]
        ...
        self.build_graph = [{},[self.starting_wf for x in range(len(self.extension_sample))]]
    ...
    def test_build(self):
        ...
        while current_length < self.max_residues and len(build_path) < (self.choice_limit*2):
            ...
            while not valid_ext:
                ...
                trial_ext = extend_helix(build,ext_length+self.min_ext_length)
                ...
                tries += 1
                # if number of tries exceeds limit, add to build path anyway and quit
                if tries >= (self.ext_try_limit*((len(build_path)/2)+1))+self.loop_try_initial: # variable trials
            ...
            while not valid_loop:
                ...
                tries += 1
                if tries >= (self.loop_try_limit*((len(build_path)/2)+1))+self.loop_try_initial:
            ...
            if trial_loop_bin not in self.call_graph_path(build_path)[0]:
                t1 = self.call_graph_path(build_path)[0][trial_loop_bin] = [{},[self.starting_wf for x in range(len(self.extension_sample))]]
            ...
            for bind,bd in enumerate(sorted_best_builds):
                if dump_ct < max_dump_per_trial:
                    ...
                    if pc_scn >= 0.18 and motif_hits >= 0.9 and rif_score >= 25 and int_sasa >= 750:
                        ...
                        nu.dump_npdb(bd,f'outputs/build_{str(pc_scn)[:8]}_{str(motif_hits)[:8]}_{str(rif_score)[:8]}_{str(int_sasa)[:8]}_{str(score_weight)[:8]}.pdb')
                        dump_ct += 1
