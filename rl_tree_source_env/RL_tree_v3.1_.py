...
class tree_builder():
    def __init__(self, stub, wnum_hash):
        ...
        self.max_ext_length = 22 # loops have partial helices on them, 4*2 loops generally
        self.min_ext_length = 6 # this means min and max have additional loop lengths
        self.max_helices = 7
        self.min_helices = 3
        ...
        # weight params
        self.weight_factor = 1 # change weighting (better handled with score function)
        self.starting_wf = 10 # starting weights (changes how quickly choices become dominant)
        ...
        # try/attempt params
        self.try_initial = 2 # starting try limit
        self.try_scalar = 1.5 # scales with distance down tree
        ...
        self.initial_weights['start'] =                             [self.ext_start_wf for x in range(self.min_ext_length,self.max_ext_length+1)] +                             [0 for x in binned_loops] +                             [0 for x in range(self.min_ext_length,self.max_ext_length+1)] +                             [0 for x in binned_loops] +                             [0]
        self.initial_weights['C_e'] =                             [0 for x in range(self.min_ext_length,self.max_ext_length+1)] +                             [self.starting_wf for x in binned_loops] +                             [0 for x in range(self.min_ext_length,self.max_ext_length+1)] +                             [self.starting_wf for x in binned_loops] +                             [0]
        self.initial_weights['C_l'] =                             [self.ext_start_wf for x in range(self.min_ext_length,self.max_ext_length+1)] +                             [0 for x in binned_loops] +                             [0 for x in range(self.min_ext_length,self.max_ext_length+1)] +                             [0 for x in binned_loops] +                             [0]
        self.initial_weights['N_e'] =                             [0 for x in range(self.min_ext_length,self.max_ext_length+1)] +                             [self.starting_wf for x in binned_loops] +                             [0 for x in range(self.min_ext_length,self.max_ext_length+1)] +                             [self.starting_wf for x in binned_loops] +                             [0]
        self.initial_weights['N_l'] =                             [0 for x in range(self.min_ext_length,self.max_ext_length+1)] +                             [0 for x in binned_loops] +                             [self.ext_start_wf for x in range(self.min_ext_length,self.max_ext_length+1)] +                             [0 for x in binned_loops] +                             [0]
        
        # set up build graph with first choice options, weights
        self.build_graph = [{},self.initial_weights['start']]
    ...
    def upweight(self, path, factor=0.1):
        ...
        for path_index,choice_name in enumerate(path):
            ...
            if (path_index+1)%2 == 0:
                ...
                t1 = self.call_graph_path(path[:path_index-1])[1] # access build path weights
                
                # calc upweight cutoff based on position in path
                uw_cutoff = min(self.upweight_cutoff + (0.025*(path_index-1)/2),0.6)
                
                if (t1[int(wt_ind)]+int(factor*len(t1))) < (sum(t1)*uw_cutoff):
                    t1[int(wt_ind)] += int(factor*len(t1))
                else:
                    t1[int(wt_ind)] = (sum(t1)*uw_cutoff)
    ...
    def test_build(self): # v3
        ...
        while current_length < self.max_residues and current_helices < self.max_helices:
            ...
            while not valid_choice:
                ...
                # pick a choice, align it, keep track of end (N or C)
                ...
                if choice_type == 'e':
                    ...
                    choice = align_loop(build, choice_end, rand_loop)
                ...
                if current_length + round(len(choice)/5) <= self.max_residues:
                    if check_hash(choice, self.wnum_hash):
                        if check_clash(build,choice_end,choice,clash_threshold=self.clash_threshold,                                       diameter_threshold=self.diam_threshold):
                            valid_choice = True
                ...
                if ((tries >= (self.try_scalar*current_helices+self.try_initial)) and (not valid_choice))                                                        or (choice_key == 'C_e_0'):
                    ...
                    if prev_choice_type == 'l' and len(prev_choice) > 0:
                        if prev_choice_end == 'C':
                            build = build[:-len(prev_choice)]
                            build_ss = build_ss[:-round(len(prev_choice)/5)]
                        else:
                            build = build[len(prev_choice):]
                            build_ss = build_ss[round(len(prev_choice)/5):]
            ...
            if choice_end == 'C': # add to C
                build = np.append(build,choice,0)
                if choice_type == 'e':
                    build_ss = build_ss + round(len(choice)/5)*'H'
                else:
                    build_ss = build_ss + round(len(choice)/5)*'L'
            else: # add to N
                build = np.append(choice,build,0)
                if choice_type == 'e':
                    build_ss = round(len(choice)/5)*'H' + build_ss
                else:
                    build_ss = round(len(choice)/5)*'L' + build_ss
            current_length = round(len(build)/5)
            current_helices = ((len(build_path)/2)+1)/2
            
            # keep track of previous choice, end, type to trim loops
            prev_choice = choice
            prev_choice_end = choice_end
            prev_choice_type = choice_type
            ...
            if choice_key not in self.call_graph_path(build_path[:-2])[0]:
                ...
                new_weights = self.initial_weights[choice_key[:3]]
    ...
    while not final_run:
        ...
        while not start:
            ...
            if check_hash(pot_start,g.wnum_hash):
                ...
                ext = extend_helix(pot_start, 'C', 16) # reasonable?
        ...
        if max([len(x)/5 for x in g.builds[-test_iters:]]) > 40 and max(g.rif_scores) > 40:
            ...
            for bind,pre_bd in enumerate(sorted_best_builds):
                if dump_ct < max_dump_per_trial:
                    ...
                    pc_scn, motif_hits, rif_score, int_sasa, score_weight = score_build(bd,input_ss=bd_ss)

                    # could set int_sasa threshold higher -- check later
                    if pc_scn >= 0.18 and motif_hits >= 0.9 and rif_score >= 25 and int_sasa >= 750:

                        nu.dump_npdb(bd,f'outputs/build5_{str(pc_scn)[:8]}_{str(motif_hits)[:8]}_{str(rif_score)[:8]}_{str(int_sasa)[:8]}_{str(score_weight)[:8]}.pdb')
                        dump_ct += 1
...
ori_1res = tt[10:12].reshape(10,4)
