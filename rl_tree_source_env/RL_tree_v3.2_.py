...
class tree_builder():
    def __init__(self, stub, wnum_hash):
        ...
        self.min_ext_length = 4 # this means min and max have additional loop lengths
        self.max_helices = 7
        self.min_helices = 3
            ...
            for bind,pre_bd in enumerate(sorted_best_builds):
                if dump_ct < max_dump_per_trial:
                    ...
                    if pc_scn >= 0.18 and motif_hits >= 0.9 and rif_score >= 25 and int_sasa >= 750:
                        ...
                        nu.dump_npdb(bd,f'outputs/build6_{str(pc_scn)[:8]}_{str(motif_hits)[:8]}_{str(rif_score)[:8]}_{str(int_sasa)[:8]}_{str(score_weight)[:8]}.pdb')
                        dump_ct += 1
