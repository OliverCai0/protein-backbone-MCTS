...
def score_build(input_pose, input_ss=None):
    ...
    # added additional reweight -- may actually be key, adjust manually
    score_wt_pre = score_pc*score_mot*score_rif*score_sasa*0.01 
    ...
    return pc_scn, motif_score, rif_score, int_sasa, worst_helix, score_wt
...
class tree_builder():
    def __init__(self, stub, wnum_hash):
        ...
        self.worst_helices = []
        self.overall_scores = []
        self.score_lists = [self.pc_scns,self.motif_scores,self.rif_scores,self.sasas,self.worst_helices,self.overall_scores]
...
def random_init_build(runs):
    ...
    for bind,pre_bd in enumerate(sorted_best_builds):
        if dump_ct < max_dump_per_trial:
            ...
            pc_scn, motif_hits, rif_score, int_sasa, worst_hel, score_weight = score_build(bd,input_ss=bd_ss)

            if pc_scn >= 0.18 and motif_hits >= 0.9 and rif_score >= 25 and int_sasa >= 750 and worst_hel > 2.0:

                nu.dump_npdb(bd,f'outputs/buildv4_{str(pc_scn)[:6]}_{str(motif_hits)[:4]}_{str(rif_score)[:5]}_{str(int_sasa)[:5]}_{str(worst_hel)[:5]}_{str(score_weight)[:6]}.pdb')
                dump_ct += 1
