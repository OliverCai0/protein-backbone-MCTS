...
B_SASA_TG = min(sum(target_face_areas) + 385,1500)
...
class tree_builder():
    def __init__(self, stub, wnum_hash):
        ...
        self.max_helix_length = 26
        self.min_helix_length = 11
...
def random_start_from_folder(runs):

    g.__init__(ori_1res, wnum_hash)
    
    start_opts = glob.glob('../starts/*')
    start_pick = int(random.random()*len(start_opts))

    g.stub = nu.npose_from_file(start_opts[start_pick])[:5*5]
    ...
    for bind,pre_bd in enumerate(sorted_best_builds):
        if dump_ct < max_dump_per_trial:
            ...
            if pc_scn >= 0.18 and motif_hits >= 0.9 and rif_score >= 25 and int_sasa >= 750 and worst_hel >= 2.0 and core_pred >= 50:
                ...
                nu.dump_npdb(bd,f'outputs/buildg_{str(pc_scn)[:6]}_{str(motif_hits)[:4]}_{str(rif_score)[:5]}_{str(int_sasa)[:6]}_{str(worst_hel)[:5]}_{str(core_pred)[:5]}_{str(score_weight)[:6]}.pdb')
                dump_ct += 1
...
for _ in range(9999):
    random_start_from_folder(50000)
